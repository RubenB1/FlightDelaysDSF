...
# Package library installation and data loading ----------------------------------------
library(tidyr)
library(stringr)
library(tidyverse)
library(dplyr)
...
load("Copy of data15_noweather_noholidays.RData")

# A bit of data cleaning --------------------------------------------------


#Clean data - columns i will not need
...
# Plotting year delays ----------------------------------------------------

# #Removing any negative values from the the ARRIVAL DELAY data so that it does not influence our analysis.
data$ARRIVAL_DELAY[data$ARRIVAL_DELAY<0] <- 0
#Now calculating the average delay per month in order to plot it.
# Select data only for specific months
jan_data=data %>%
  filter(MONTH==1)
feb_data=data %>%
  filter(MONTH==2)
...
jan_delay= mean(jan_data$ARRIVAL_DELAY)
feb_delay= mean(feb_data$ARRIVAL_DELAY)
mar_delay=mean(mar_data$ARRIVAL_DELAY)
apr_delay=mean(apr_data$ARRIVAL_DELAY)
may_delay=mean(may_data$ARRIVAL_DELAY)
jun_delay=mean(jun_data$ARRIVAL_DELAY)                
jul_delay=mean(jul_data$ARRIVAL_DELAY)
aug_delay=mean(aug_data$ARRIVAL_DELAY)
sep_delay=mean(sep_data$ARRIVAL_DELAY)
nov_delay=mean(nov_data$ARRIVAL_DELAY)
dec_delay=mean(dec_data$ARRIVAL_DELAY)

#Creating data frame to do the plot
...
MEAN_DELAY_MONTHS= c(jan_delay, feb_delay, mar_delay, apr_delay, may_delay, jun_delay, jul_delay, aug_delay, sep_delay, nov_delay, dec_delay)


Average_Delay_month= data.frame(Months, MEAN_DELAY_MONTHS)
#We need to arrange this by months

Average_Delay_month$Months <-factor(Average_Delay_month$Months, 
...
#PLOTTING
PLOT_TD_ZERO_ADV=ggplot(Average_Delay_month, aes(x = Months, y = MEAN_DELAY_MONTHS)) +
  geom_bar(stat = "identity", position = "dodge", color = "black", fill="violetred4")+
  theme_bw()+
  theme(axis.text = element_text(size = 8), 
        axis.title = element_text(size = 18, face = "bold"),
        plot.title = element_text(size = 24, face = "bold"))+
  labs(x=  element_blank(), y = "Average delay(min)" ,
       title = "Average flight delays per month", ps = 12, cex = 1, cex.main = 1)



# Day of week delay -------------------------------------------------------

#Lets check out how delays vary according to the day of the week
#creating data frame to plot the results
mon_data=data %>%
  filter(DAY_OF_WEEK==1)
tue_data=data %>%
  filter(DAY_OF_WEEK==2)
wed_data=data %>%
  filter(DAY_OF_WEEK==3)
thu_data=data %>%
...
mon_delay= mean(mon_data$ARRIVAL_DELAY)
tue_delay= mean(tue_data$ARRIVAL_DELAY)
wed_delay= mean(wed_data$ARRIVAL_DELAY)
thu_delay= mean(thu_data$ARRIVAL_DELAY)
fri_delay= mean(fri_data$ARRIVAL_DELAY)
sat_delay= mean(sat_data$ARRIVAL_DELAY)
sun_delay= mean(sun_data$ARRIVAL_DELAY)
